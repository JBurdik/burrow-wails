  ...
  <main id="main-content" class="dashboard-body">
    ...
    <template v-else>
      <section v-if="store.chats.length" class="session-section" aria-labelledby="chat-heading">
        ...
        <ul class="session-list"><li v-for="chat in store.chats.slice(0, 3)" :key="chat.id"><button class="session-row" type="button" @click="store.openChat(chat)"><span :class="['session-icon', { running: chat.busy }]" aria-hidden="true">✦</span><span class="session-main"><span class="session-title">{{ chat.title }}</span><span class="session-path">{{ chat.agentKind || chat.transport }}</span></span><span :class="['state-pill', { running: chat.busy }]">{{ chat.busy ? 'Pracuje' : 'Připraven' }}</span><span class="session-chevron" aria-hidden="true">›</span></button></li></ul>
      </section>
