...
<template>
  <header class="dashboard-header">
    ...
    <!-- Truthful about the socket, not just about the last fetch: the
         transport reports going down, so a stale screen says so instead of
         claiming "Připojeno" over a dead connection. -->
    <button
      class="connection"
      :class="{ 'connection--down': !store.connected }"
      type="button"
      :disabled="store.loading"
      @click="store.loadSessions"
      aria-label="Obnovit relace"
    >
      <span class="connection-dot" aria-hidden="true" />{{
        !store.connected ? (store.reconnecting ? 'Obnovuji spojení' : 'Odpojeno')
        : store.loading ? 'Obnovuji' : 'Připojeno'
      }}
    </button>
  </header>
  ...
  <main id="main-content" class="dashboard-body">
    <section class="summary" aria-labelledby="now-heading">
      <div>
        ...
        <p class="summary-line"><strong>{{ liveChats.length }}</strong> {{ liveChats.length === 1 ? 'agent pracuje v chatu' : liveChats.length < 5 ? 'agenti pracují v chatu' : 'agentů pracuje v chatu' }}</p>
