...
<template>
  ...
  <main class="m-body chat-list">
    <section class="new-chat"><p class="eyebrow">NOVÁ KONVERZACE</p><div class="create-grid"><select v-model="workspaceId"><option :value="null" disabled>Vyber projekt…</option><option v-for="workspace in store.workspaces" :key="workspace.id" :value="workspace.id">{{ workspace.name }}</option></select><select v-model="agentKind"><option value="codex">Codex</option><option value="claude">Claude</option></select><button type="button" :disabled="!workspaceId || creating" @click="createChat">{{ creating ? 'Spouštím…' : 'Nový chat' }}</button></div><p v-if="createError" class="create-error">{{ createError }}</p></section>
