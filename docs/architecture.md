# Burrow architecture — detail

> Extracted from `CLAUDE.md` (which hit the context limit). `CLAUDE.md` keeps a one-paragraph
> summary per subsystem; this file keeps the full rationale — *why* each thing is built the way it
> is, and which bug it exists to prevent. Read the matching section here before changing a subsystem.
...
**`parent_chat_id` is what makes a sub-agent belong to a thread rather than sit beside
it.** A `spawn` made from inside a chat (carrying `parent_chat_id`, injected from
`BURROW_CHAT_ID` by `bin/burrow` and `burrow-mcp` alike — see `docs/burrow.html`) creates a
child row with that column set instead of a new
top-level chat or tab, and three surfaces key off it rather than off a separate
parent/child table: the Sidebar's `sessionsForWs` filters every list it builds on
`!parentChatId`, so a child never appears as a peer of the thread that spawned it; the
Right Panel's Sub-agents surface scopes to `childrenOf(sessions, activeChatId)`, so it
shows exactly one thread's children and nothing from a sibling thread; and `DeleteChat`
cascades a `DELETE FROM chats WHERE parent_chat_id = ?` before deleting the row itself, so
a thread's children can never outlive it as orphaned rows nothing can reach. Depth is
capped at **one level** — a sub-agent's own `spawn` is refused with `sub-agent cannot
spawn sub-agents` — and the guard is derived server-side (`chatIsSubagent`, looked up
fresh from the caller's own `parent_chat_id` on every call) rather than trusted from the
request body, because a client-supplied `caller_is_subagent: false` would otherwise let a
sub-agent's own transcript talk it into spawning a grandchild the app has no plan for
tracking, displaying, or tearing down.
...
**`src/machines/agentStatus.ts` still exists — it was not deleted.** Terminals are fully off it (replaced by the phase + `displayStatus` above). `src/stores/claudeChats.ts` and `AgentChat.vue` still drive one instance of it per chat session, because chat permission state today arrives on the control/permission protocol (a pending `control_request`: generic tool/file edit → `permission`+bell, `AskUserQuestion`/`ExitPlanMode` → `waiting`) rather than as a phase — a top-level chat's own status still has no phase-driven path, only its children's do (see the Sub-agents surface above). Read it as chat-only, not dead code.
...
not part of the neutral event vocabulary. A chat's own status still comes from
`busy`/`pendingPermission` rather than `phase-chat:` on both clients — the desktop's
Right Panel now reads `phase-chat:` too, but only for a thread's sub-agent children, not
for a top-level chat's own status, so this is still true for the surface it was written
about. `store.ts` was **not** deleted and mobile was **not** moved onto the desktop's
...
preset actually have examples). `ui.textGenerationRules` is a free-text house rule
appended to the **commit and PR** prompts only — a preset says *which* conventions,
this says the ones no preset can know (ticket prefix, language, banned words); empty
adds nothing. All three live in Settings → General; **`src/stores/git.ts`
