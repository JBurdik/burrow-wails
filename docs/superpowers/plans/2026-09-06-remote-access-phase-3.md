...
- Produces:
  ...
  - `func callApp(app *App, c remoteCmd, args map[string]json.RawMessage) (any, error)`
...
func TestCallAppCoercesNumberToString(t *testing.T) {
	...
	app := &App{}
	_, err := callApp(app, remoteCmd{Method: "GetPtyForeground", Args: []string{"id"}},
...
func TestCallAppRejectsWrongType(t *testing.T) {
	app := &App{}
	_, err := callApp(app, remoteCmd{Method: "SetMaxAgents", Args: []string{"n"}},
...
func TestCallAppRejectsUnknownMethod(t *testing.T) {
	app := &App{}
	if _, err := callApp(app, remoteCmd{Method: "NoSuchMethod"}, nil); err == nil {
		t.Fatal("unknown method must be an error")
	}
}

func TestCallAppSplitsTrailingError(t *testing.T) {
	// ListPtySessions returns ([]string, error) with no daemon attached, so
	// the error must surface as the call's error, not as a result value.
	app := &App{}
	_, err := callApp(app, remoteCmd{Method: "ListPtySessions", Args: nil},
		map[string]json.RawMessage{})
	if err == nil {
...
func callApp(app *App, c remoteCmd, args map[string]json.RawMessage) (any, error) {
	m := reflect.ValueOf(app).MethodByName(c.Method)
	if !m.IsValid() {
...
func TestWSCallGetsAReply(t *testing.T) {
	...
	if err := conn.WriteJSON(clientFrame{T: "call", ID: 1, Cmd: "get_pty_foreground"}); err != nil {
		t.Fatal(err)
	}
	var f serverFrame
