...
- `burrow spawn "<task>" [--agent NAME] [--model ID] [--cwd DIR] [--target tab|chat]` — delegate a task to a sub-agent in a new visible tab, fire-and-forget. It takes the TASK, not a command line: say what to do, which files, what not to touch, what to report. It already runs in your workspace's directory — do not restate the repo path or `cd` anywhere, say "this repo". Prints the tab's `pty_id` and a result `token`.
...
Nothing wakes you when a sub-agent finishes — no notification, no auto-resume. In a terminal tab, fan out and collect later. In a CHAT thread your turn ends, and there is no "later" without a user prompt: `burrow wait --chat-id <id>` on each child, or `collect-results` and say plainly which ones are still running. Never end a turn claiming a result will arrive by itself. Sub-agents run interactively on the subscription (never `codex exec` / `claude -p`). `run` is read-only — delegate anything that changes files. Destructive verbs (`tab-close`, `worktree-remove`, `pr-merge`) need the user's explicit confirmation first.
